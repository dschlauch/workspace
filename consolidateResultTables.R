...
baseDir <- "~/gd/Harvard/Research/TM_outputs/CISPB/BERE/"
outputDir <- './'
setwd(baseDir)
...
makeComparisonPlot <- function(pair, plotTopNTFs=15, filterColIndices = c(8,18,28,38), metric="Magnitude"){
  ...
  plot1 <- plot1 + geom_point(colour="blue",alpha=.5, size=4) + xlab(displayNames[pair[1]]) + ylab(displayNames[pair[2]]) + 
    geom_text(vjust=0)  + expand_limits(x=c(0,.025)) + annotate("text", x = .02, y = 0, ,hjust=0,label = corText, parse = TRUE)
  ggMarginal(plot1)
}
...
generatePlots <- function(metric, filterColIndices){
  ...
  plot1 <- suppressWarnings(makeComparisonPlot(c(1,2),metric=metric, filterColIndices=filterColIndices))
  plot2 <- suppressWarnings(makeComparisonPlot(c(3,4),metric=metric, filterColIndices=filterColIndices))
  ...
  png(paste0(outputDir, metric, '_comparison_same_tissue.png'), width=900, height=450)
  suppressWarnings(grid.arrange(plot1, plot2, ncol=2, top="Comparison of Differential TF Involvement Across Studies of Same Tissue"))
  dev.off()
  png(paste0(outputDir, metric, '_comparison_diff_tissue.png'), width=900,height=750)
  suppressWarnings(grid.arrange(plot3, plot4, plot5, plot6, ncol=2, top="Comparison of Differential TF Involvement Across Studies of Different Tissues",
                                left = textGrob("Lung Tissue", rot = 90, vjust = 1), bottom = textGrob("Blood")))
  dev.off()
}
